...
namespace KinveyXamarin
{
	public class ExpressionNormalizer : ExpressionVisitors
	{
		...
		public ExpressionNormalizer (Expression exp)
		{
			_expression = exp;
		}
		...
		protected override Expression VisitBinary(BinaryExpression node)
		{
			...
			if (node.NodeType == ExpressionType.Coalesce)
			{
				...
				if (node.Left.NodeType == ExpressionType.Equal &&
					(node.Left.Type).IsConstructedGenericType && //TODO required method doesnt exist, not sure what constructed means in this context
